...
app.controller('qualificationCtrl',function($http, $scope, $timeout,$q) {
    ...
    if(!state) {
        $scope.index = parents.authList('myqualificationmanager-index');//查看列表权限
        $scope.read = parents.authList('myqualificationmanager-read');//查看详情权限
        $scope.update = parents.authList('myqualificationmanager-update');//编辑操作
        $scope.save = parents.authList('myqualificationmanager-save');//添加操作
        $scope.delete = parents.authList('myqualificationmanager-delete');//删除操作
        $scope.deletes = parents.authList('myqualificationmanager-deletes');//批量删除操作
        $scope.enables = parents.authList('myqualificationmanager-enables');//批量启用禁用操作
        $scope.isexport = parents.authList('myqualificationmanager-export');//导出操作
        $scope.imports = parents.authList('myqualificationmanager-import');//导入操作
    }
    $("#query").show();

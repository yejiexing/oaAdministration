 ...
 */
var app = angular.module('opeBusiness',[]);
app.controller('opeBusinessCtrl',function($http, $scope, $timeout,$q) {
    var self = this;
    ...
    if(!state) {
        $scope.index = parents.authList('business-index');//查看列表权限
        $scope.read = parents.authList('business-read');//查看详情权限
        $scope.update = parents.authList('business-update');//编辑操作
        $scope.save = parents.authList('business-save');//添加操作
        $scope.delete = parents.authList('business-delete');//删除操作
        $scope.deletes = parents.authList('business-deletes');//批量删除操作
        $scope.enables = parents.authList('business-enables');//批量启用禁用操作
        $scope.isexport = parents.authList('business-exportdata');//导出操作
        $scope.imports = parents.authList('business-importdata');//导入操作
        $scope.tobusiness = parents.authList('business-copyoperatetobusiness');//导入操作
    }
    $("#query").show();
        ...
        $http.get(url + 'admin/business/getOperateData', {
            ...
            function(data) {
                if (data.data.code === 200) {
                    ...
                    console.log($scope.copysettlementArr)
    ...
    $scope.editFun = function(params){//编辑
        $scope.usersError = '';
        $scope.Id = $scope.copybusinessArr.id;
        $scope.Method = 'put';
        $http.defaults.headers.common = { token: parents.token() };
        $http({
            method: 'get',

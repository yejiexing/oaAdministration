 ...
 */
var app = angular.module('dataSummary', []);
app.controller('dataSummaryCtrl', function($http, $scope) {
    var self = this;
    ...
    if (!state) {
        $scope.index = parents.authList('summary-index'); //查看列表权限
        $scope.read = parents.authList('summary-read'); //查看详情权限
        $scope.update = parents.authList('summary-update'); //编辑操作
        $scope.save = parents.authList('summary-save'); //添加操作
        $scope.delete = parents.authList('summary-delete'); //删除操作
        $scope.deletes = parents.authList('summary-deletes'); //批量删除操作
        $scope.enables = parents.authList('summary-enables'); //批量启用禁用操作
        $scope.isexport = parents.authList('summary-exportsummary');//导出操作
    }
    $('#query').show();
    ...
    console.log( parents.select('project/getProjectList'));

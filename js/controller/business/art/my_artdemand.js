 ...
 */
var app = angular.module('myartDemand', []);
app.controller('myartDemandCtrl', function($http, $scope, $timeout) {
    var self = this;
    ...
    if (!state) {
        $scope.index = parents.authList('myfineart-index'); //查看列表权限
        $scope.read = parents.authList('myfineart-read'); //查看详情权限
        $scope.update = parents.authList('myfineart-update'); //编辑操作
        $scope.save = parents.authList('myfineart-save'); //添加操作
        $scope.delete = parents.authList('myfineart-delete'); //删除操作
        $scope.deletes = parents.authList('myfineart-deletes'); //批量删除操作
        $scope.enables = parents.authList('myfineart-enables'); //批量启用禁用操作
        $scope.demand = parents.authList('myfineart-finishdemand'); //完成操作
        $scope.isexport = parents.authList('myfineart-export');//导出操作
    }
    $('#query').show();
            ...
            .success(function(data) {
                if (data.code == 200) {
                    ...
                    if (params == 1) {
                        ...
                        $scope.Channel_s = parents.select(
                            'project/getProjectNameAndId'
                        );
                        parents.forSome(
                            $scope.copyimageArr,
    ...
    $scope.audit = function(status) {
        $http.defaults.headers.common = { token: parents.token() };
        $http.post(url + 'admin/fine_art/finishDemand' ,{id: $scope.imageArr.id}).then(
            function(data) {
                $('#audit').modal('hide');
                $('#code').html(data.data.data);
                $('#table_success').modal('show');
    ...
    $scope.demand1 = function(params) {
        if($scope.copyimageArr.status == 0) {
            $('#audit').modal('show');
        }else{
            alert('当前需求已完成');
        }
    ...
    $scope.setProject = function() {
        $scope.Channel_s.forEach(function(item) {
            if (item.id == $scope.copyimageArr.project_id) {
                $scope.copyimageArr.project_name = item.name;
            }
        });
    };
            ...
            onLoadSuccess: function(data) {
                ...
                if (data.total != 0) {
                    ...
                    var tap = function() {
                        ...
                        $scope.deleteState = $scope.editState = $scope.iconState = $scope.auditState = true;
                        $scope.imageArr = $scope.copyimageArr = data.rows[t_idx];

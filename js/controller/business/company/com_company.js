 ...
 */
var app = angular.module('comCompany',[]);
app.controller('comCompanyCtrl',function($http, $scope, $timeout) {
    var self = this;
    ...
    if(!state) {
        $scope.index = parents.authList('contentprovider-index');//查看列表权限
        $scope.read = parents.authList('contentprovider-read');//查看详情权限
        $scope.update = parents.authList('contentprovider-update');//编辑操作
        $scope.save = parents.authList('contentprovider-save');//添加操作
        $scope.delete = parents.authList('contentprovider-delete');//删除操作
        $scope.deletes = parents.authList('contentprovider-deletes');//批量删除操作
        $scope.enables = parents.authList('contentprovider-enables');//批量启用禁用操作
        $scope.isexport = parents.authList('contentprovider-export');//导出操作
        $scope.imports = parents.authList('contentprovider-import');//导入操作
    }
    $("#query").show();
    ...
    $scope.Id = '';
    self.Channel_s = '';//选择CP名称-搜索
    self.Company_s = '';//选择单位-搜索
    $scope.select = {
        selectChannel : '', //选择CP名称
        selectCompany : '', //选择单位
        C_status: true
    }
    $scope.Method = '';
    $scope.errorstate = '';
    $scope.stateIndex = 0;
    $scope.companyArr = $scope.copycompanyArr = new Object();
    $scope.checks = [];
    $scope.ChannelList = parents.select('content_provider/getContentProviderList');//选择CP名称list
    $scope.CompanyList = parents.select('content_provider/getCompanyList');//获取单位list

    initTable();
    $scope.doQuery = function(params){
        $('#demo-table').bootstrapTable('refresh');    //刷新表格
        $scope.ChannelList = parents.select('content_provider/getContentProviderList');//刷新CP名称list
        $scope.CompanyList = parents.select('content_provider/getCompanyList');//刷新单位list
    ...
    $scope.editFun = function(params){//编辑
        $scope.usersError = '';
        $scope.Id = $scope.companyArr.id;
        $scope.Method = 'put';
        $http.defaults.headers.common = { token: parents.token() };
        $http({
            method: 'get',
            url: url + 'admin/content_provider/' + $scope.Id
        }).success(function (data) {
            if(data.code == 200) {
                $scope.copycompanyArr = data.data;
                if(params == 1){
                    $scope.dealList = $scope.copycompanyArr.dealList;
    ...
    $scope.export = function(){//导出
        var ExpObj = {
            provider_name : self.Channel_s,
            company_name : self.Company_s
        }
        parents.Exports('content_provider/export',ExpObj)
    };
    ...
    $scope.deleteConfirm = function() {
        ...
        if (checks.length) {
            $http.post(url + 'admin/content_provider/deletes', {
                ids: checks
            }).then(
                function(data) {
            ...
            $http
                .delete(url + 'admin/content_provider/' + $scope.companyArr.id)
                .then(
                    function(data) {
                        $('#table_delete').modal('hide');
                        $('#code').html(data.data.data);
    ...
    $scope.preservaConfirm = function(v){//新增弹出框
        var channelNum = parents.forEachs($scope.copycompanyArr);
        if(channelNum != 0){
            $scope.usersError = '请完善必填内容';
            return false;
        }
        parents.screen.Delete($scope.copycompanyArr)
        $http.defaults.headers.common = { token: parents.token() };
        $http({
            method : $scope.Method,
            url: url + 'admin/content_provider/' + $scope.Id,
            params : $scope.copycompanyArr
        }).success(function(data){
            if(data.code == 200){
                $scope.doQuery();
                if(v == 1) {
                ...
                $http
                    ...
                    function (data) {
                        if (data.data.code === 200) {
                            //$scope.copycompanyArr.statement_file_name = file.name;
                            $scope.copycompanyArr.license = data.data.data;
    ...
    $scope.NewlyAdded = function(){
        ...
        $('input[type="file"]').val(null);
        $("#preservaModal").modal("show");
        $("#myModalLabel").html('[新增]cp公司');
    }
    $scope.charges = function(v){
        $("#charge").modal("show");
        $("#chargeTitle").html("请选择...");
        $scope.copyp_name = v == 1?angular.copy($scope.copycompanyArr.finance_incharge_id):angular.copy($scope.copycompanyArr.business_incharge_id);
        $scope.copyId = v;
    ...
    $scope.delp_name = function(v){//负责人清空
        if(v == 1){
            $scope.copycompanyArr.finance_incharge = '';
            $scope.copycompanyArr.finance_incharge_id = '';
        }else {
            $scope.copycompanyArr.business_incharge_id = '';
            $scope.copycompanyArr.business_incharge = '';
        }
    };
    $scope.chargesKeep = function(){
        var depar = $("input[name='charge']:checked");
        if(depar.val() != undefined) {
            $scope.copycompanyArr.finance_incharge = depar.attr('data-num');
            $scope.copycompanyArr.finance_incharge_id = depar.val();
            $("#charge").modal("hide");
        }else{
            alert('请选择负责人');
        }
    ...
    function initTable(){
        $('#demo-table').bootstrapTable({
            ...
            url:url + 'admin/content_provider',
            height: 670,
            showColumns:false,
            pagination:true,
            showRefresh:false,
            ...
            onLoadSuccess:function(data){ //成功的回调
                ...
                var tap = function() {
                    ...
                    $scope.companyArr = $scope.copycompanyArr = data.rows[t_idx];
    ...
    $scope.stateedit = function(v){
        ...
        $scope.companyArr.status = pop_che ? 1: 0;
        $http.defaults.headers.common = { token: parents.token() };
        $http({
            method : "put",
            url: url + 'admin/users/' + $scope.companyArr.id,
            params : $scope.companyArr
        }).success(function(data){
            if(data.code == 200){
                if(pop_che){

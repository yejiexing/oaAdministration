...
app.controller('proIntroductionCtrl',function($http, $scope, $timeout) {
    ...
    if(!state) {
        $scope.index = parents.authList('myproductintroduction-index');//查看列表权限
        $scope.read = parents.authList('myproductintroduction-read');//查看详情权限
        $scope.update = parents.authList('myproductintroduction-update');//编辑操作
        $scope.save = parents.authList('myproductintroduction-save');//添加操作
        $scope.delete = parents.authList('myproductintroduction-delete');//删除操作
        $scope.deletes = parents.authList('myproductintroduction-deletes');//批量删除操作
        $scope.enables = parents.authList('myproductintroduction-enables');//批量启用禁用操作
        $scope.isexport = parents.authList('myproductintroduction-export');//导出操作
        $scope.imports = parents.authList('myproductintroduction-import');//导入操作
        $scope.eval = parents.authList('myproductintroduction-updatestatustoevaluation');//导入操作
        $scope.selectTester = parents.authList('myproductintroduction-selecttester');//导入操作
        $scope.finishedTest = parents.authList('myproductintroduction-finishedtest');//导入操作
        $scope.archive = parents.authList('myproductintroduction-archive');//导入操作
    }
    $("#query").show();

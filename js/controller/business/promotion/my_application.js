...
app.controller('myApplicationCtrl',function($http, $scope, $timeout) {
    var self = this;
    ...
    if(!state) {
        $scope.index = parents.authList('mypromotionapplication-index');//查看列表权限
        $scope.read = parents.authList('mypromotionapplication-read');//查看详情权限
        $scope.update = parents.authList('mypromotionapplication-update');//编辑操作
        $scope.save = parents.authList('mypromotionapplication-save');//添加操作
        $scope.delete = parents.authList('mypromotionapplication-delete');//删除操作
        $scope.deletes = parents.authList('mypromotionapplication-deletes');//批量删除操作
        $scope.enables = parents.authList('mypromotionapplication-enables');//批量启用禁用操作
        $scope.isexport = parents.authList('mypromotionapplication-export');//导出操作
        $scope.demand = parents.authList('mypromotionapplication-verifyapplication');
        $scope.finishs = parents.authList('mypromotionapplication-finishapplication');
        $scope.updateinvoiceinfo = parents.authList('mypromotionapplication-updateinvoiceinfo');
        $scope.updatebudgetamount = parents.authList('mypromotionapplication-updatebudgetamount');
        $scope.updateconsumedamount = parents.authList('mypromotionapplication-updateconsumedamount');
    }
    $("#query").show();
    ...
    $scope.editFun = function(params){//编辑
        ...
        $http({
            ...
            if(data.code == 200) {
                ...
                if(params == 1){
                    $scope.dealList = $scope.copyapplicationArr.dealList;

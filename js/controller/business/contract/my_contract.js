 ...
 */
var app = angular.module('myContract',[]);
app.controller('myContractCtrl',function($http, $scope, $timeout,$q) {
    var self = this;
    ...
    if(!state) {
        $scope.index = parents.authList('mycontract-index');//查看列表权限
        $scope.read = parents.authList('mycontract-read');//查看详情权限
        $scope.update = parents.authList('mycontract-update');//编辑操作
        $scope.save = parents.authList('mycontract-save');//添加操作
        $scope.delete = parents.authList('mycontract-delete');//删除操作
        $scope.deletes = parents.authList('mycontract-deletes');//批量删除操作
        $scope.enables = parents.authList('mycontract-enables');//批量启用禁用操作
        $scope.isexport = parents.authList('mycontract-export');//导出操作
        $scope.imports = parents.authList('mycontract-import');//导入操作
    }
    $("#query").show();
    ...
    $scope.TypeList = [];
    $scope.getTypeList = parents.select('contract/getTypeList?mydata=1');
        ...
        $http({
            ...
            if(data.code == 200) {
                ...
                if(params == 1){
                    ...
                    parents.forSome($scope.copycontractArr, ['type', 'project_id', 'product_id'], String);
    ...
    $scope.preservaConfirm = function(v){//新增弹出框
        if(v == 1){
            ...
            angular.forEach($scope.copycontractArr, function (v, i) {
                if ((i == 'number' || i == 'type' || i == 'sign_date' || i == 'start_date' || i == 'end_date') && v == '') {
                    channelNum++;
                }
            })
            if (channelNum != 0) {

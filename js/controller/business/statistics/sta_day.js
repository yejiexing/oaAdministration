 ...
 */
var app = angular.module('statisticsDay', []);
app.controller('statisticsDayCtrl', function($http, $scope) {
    var self = this;
    ...
    if (!state) {
        ...
        $scope.isexport = parents.authList('statistics-exportdaycount');//导出操作
    }
    $('#query').show();
    ...
    $scope.ChannelList = parents.select('statistics/getProjectList?field=date'); //获取单位list
    $scope.RegionList = parents.select('statistics/getChannelList?field=date'); //获取单位list

    initTable();
    $scope.doQuery = function(params) {
        $('#demo-table').bootstrapTable('refresh'); //刷新表格
        $scope.ChannelList = parents.select('statistics/getProjectList?field=date'); //刷新渠道list
    ...
    $scope.setRegion = function(item) {
        if(item != '*暂无上线渠道*') {
            ...
            $scope.Region = item === '渠道明细' ? 'detail' : item === '渠道汇总' ? 'all' : item;
        }
    };
    ...
    function initTable() {
        $('#demo-table').bootstrapTable({
            ...
            url: url + 'admin/statistics/getDayCount',
            height: 670,
            showColumns: false,
            pagination: true,
            showRefresh: false,

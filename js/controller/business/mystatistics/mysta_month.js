 ...
 */
var app = angular.module('mystatisticsMonth', []);
app.controller('mystatisticsMonthCtrl', function($http, $scope) {
    var self = this;
    ...
    if (!state) {
        ...
        $scope.isexport = parents.authList('mystatistics-exportmonthcount');//导出操作
    }
    $('#query').show();
    ...
    $scope.ChannelList = parents.select('statistics/getProjectList?my_data=true&field=month'); //获取单位list
    $scope.RegionList = parents.select('statistics/getChannelList?my_data=true&field=month'); //获取单位list

    initTable();
    $scope.doQuery = function(params) {
        $('#demo-table').bootstrapTable('refresh'); //刷新表格
        $scope.ChannelList = parents.select('statistics/getProjectList?my_data=true&field=month'); //刷新渠道list
    ...
    $scope.stateedit = function(v) {
        ...
        $http({
            ...
            url:
                url +
                'admin/statistics/getMonthCount/' +

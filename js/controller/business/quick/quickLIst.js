...
app.controller('quickListCtrl',function($http, $scope, $timeout) {
    var self = this;
    ...
    $scope.getExcl = function(){
        ...
        $http({
            ...
            if(data.code == 200){
                ...
                parents.OpenDemo('index.php/'+data.data)
            }else{
    ...
    $scope.deleteConfirm = function() {
        ...
        if (checks.length) {
            $http.post(url + 'admin/quick/deletes', {
                ids: checks
            }).then(
                function(data) {
            ...
            $http
                .delete(url + 'admin/quick/' + $scope.copyquickArr.id)
                .then(
                    function(data) {
    ...
    $scope.preservaConfirm = function(){//新增弹出框
        ...
        $http({
            method : 'PUT',
            url: url + 'admin/quick/'+$scope.copyquickArr.id,
            params : {
                appid : $scope.copyquickArr.appid,
                appkey : $scope.copyquickArr.appkey,
                product_code : $scope.copyquickArr.product_code,
                channel_code : $scope.copyquickArr.channel_code
            }
        }).success(function(data){
            if(data.code == 200){
                $scope.doQuery();
    ...
    $scope.import = function() {
        ...
        $http
            .post(url + 'admin/quick/import', fd, {

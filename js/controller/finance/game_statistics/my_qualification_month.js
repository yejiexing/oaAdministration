 ...
 */
var app = angular.module('myStatisticsQualificationMonth', []);
app.controller('myStatisticsQualificationMonthCtrl', function($http, $scope) {
    var self = this;
    ...
    if (!state) {
        $scope.index = parents.authList('getqualificationmonthcount-index'); //查看列表权限
        $scope.read = parents.authList('getqualificationmonthcount-read'); //查看详情权限
        $scope.update = parents.authList('getqualificationmonthcount-update'); //编辑操作
        $scope.save = parents.authList('getqualificationmonthcount-save'); //添加操作
        $scope.delete = parents.authList('getqualificationmonthcount-delete'); //删除操作
        $scope.deletes = parents.authList('getqualificationmonthcount-deletes'); //批量删除操作
        $scope.enables = parents.authList('getqualificationmonthcount-enables'); //批量启用禁用操
        $scope.isexport = parents.authList('myexpensestatistics-exportqualificationmonthcount');//导出操作
    }
    $('#query').show();
    ...
    $scope.export = function() {
        ...
        parents.Exports(
            'expense_statistics/exportQualificationMonthCount',
            ExpObj
        );
    };
    ...
    function initTable() {
        $('#demo-table').bootstrapTable({
            ...
            url: url + 'admin/expense_statistics/getQualificationMonthCount',
            height: 670,
            showColumns: false,
            pagination: true,
            showRefresh: false,
    ...
    $scope.stateedit = function(v) {
        ...
        $http({
            ...
            url:
                url +
                'admin/expense_statistics/getQualificationMonthCount/' +

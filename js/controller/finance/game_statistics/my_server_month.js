 ...
 */
var app = angular.module('myStatisticsServerMonth', []);
app.controller('myStatisticsServerMonthCtrl', function($http, $scope) {
    var self = this;
    ...
    if (!state) {
        $scope.index = parents.authList('getservermonthcount-index'); //查看列表权限
        $scope.read = parents.authList('getservermonthcount-read'); //查看详情权限
        $scope.update = parents.authList('getservermonthcount-update'); //编辑操作
        $scope.save = parents.authList('getservermonthcount-save'); //添加操作
        $scope.delete = parents.authList('getservermonthcount-delete'); //删除操作
        $scope.deletes = parents.authList('getservermonthcount-deletes'); //批量删除操作
        $scope.enables = parents.authList('getservermonthcount-enables'); //批量启用禁用操作
        $scope.isexport = parents.authList('myexpensestatistics-exportservermonthcount');//导出操作
    }
    $('#query').show();
    ...
    $scope.export = function() {
        ...
        parents.Exports('expense_statistics/ExportserverMonthCount', ExpObj);
    };
    ...
    function initTable() {
        $('#demo-table').bootstrapTable({
            ...
            url: url + 'admin/expense_statistics/getServerMonthCount',
            height: 670,
            showColumns: false,
            pagination: true,
            showRefresh: false,
    ...
    $scope.stateedit = function(v) {
        ...
        $http({
            ...
            url:
                url +
                'admin/expense_statistics/getServerMonthCount/' +

 ...
 */
var app = angular.module('myStatisticsGameDay', []);
app.controller('myStatisticsGameDayCtrl', function($http, $scope) {
    var self = this;
    ...
    if (!state) {
        $scope.index = parents.authList('getgamedaycount-index'); //查看列表权限
        $scope.read = parents.authList('getgamedaycount-read'); //查看详情权限
        $scope.update = parents.authList('getgamedaycount-update'); //编辑操作
        $scope.save = parents.authList('getgamedaycount-save'); //添加操作
        $scope.delete = parents.authList('getgamedaycount-delete'); //删除操作
        $scope.deletes = parents.authList('getgamedaycount-deletes'); //批量删除操作
        $scope.enables = parents.authList('getgamedaycount-enables'); //批量启用禁用操作
        $scope.isexport = parents.authList('myexpensestatistics-exportgamedaycount');//导出操作
    }
    $('#query').show();
    ...
    $scope.export = function() {
        ...
        parents.Exports('expense_statistics/exportGameDayCount', ExpObj);
    };
    ...
    function initTable() {
        $('#demo-table').bootstrapTable({
            ...
            url: url + 'admin/expense_statistics/getGameDayCount',
            height: 670,
            showColumns: false,
            pagination: true,
            showRefresh: false,
    ...
    $scope.stateedit = function(v) {
        ...
        $http({
            ...
            url:
                url +
                'admin/expense_statistics/getGameDayCount/' +

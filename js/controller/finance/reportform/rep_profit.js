 ...
 */
var app = angular.module('repProfit', []);
app.controller('repProfitCtrl', function($http, $scope, $q, $timeout) {
    var self = this;
    ...
    if (!state) {
        $scope.index = parents.authList('financeprofitreport-index'); //查看列表权限
        $scope.read = parents.authList('financeprofitreport-read'); //查看详情权限
        $scope.update = parents.authList('financeprofitreport-update'); //编辑操作
        $scope.save = parents.authList('financeprofitreport-save'); //添加操作
        $scope.delete = parents.authList('financeprofitreport-delete'); //删除操作
        $scope.deletes = parents.authList('financeprofitreport-deletes'); //批量删除操作
        $scope.enables = parents.authList('financeprofitreport-enables'); //批量启用禁用操作
        $scope.isexport = parents.authList('financeprofitreport-export');//导出操作
        $scope.imports = parents.authList('financeprofitreport-import');//导入操作
        $scope.checkre = parents.authList('financeprofitreport-checkreportintime');//导入操作
    }
    $('#query').show();
    ...
    $scope.UnitList = parents.select('finance_profit_report/getIssue');
    ...
    $scope.modalSearch = function() {
        $scope.usersError = '';
        $http.get(url + 'admin/finance_profit_report/getFinanceStatement', {
                headers: {
                    token: parents.token()
                },
                params: {
    ...
    $scope.export = function() {
        ...
        parents.Exports('finance_profit_report/export', ExpObj);
    };
    ...
    $scope.editFun = function(params) {
        ...
        $http({
            method: 'get',
            url: url + 'admin/finance_profit_report/' + $scope.settlementArr.id
    ...
    $scope.deleteConfirm = function() {
        ...
        if (checks.length) {
            $http
                .post(url + 'admin/finance_profit_report/deletes', {
                    ids: checks
                })
                .then(function(data) {
            ...
            $http
                .delete(url + 'admin/finance_profit_report/' + $scope.settlementArr.id)
                .then(function(data) {
                    if (data.data.code === 200) {
                        $('#table_delete').modal('hide');
                        $('#code').html(data.data.data);
    ...
    $scope.preservaConfirm = function() {
        ...
        $http({
            method: 'post',
            url: url + 'admin/finance_profit_report',
            params: {
                start_date: $scope.copysettlementArr.start_date,
                end_date: $scope.copysettlementArr.end_date,
                project_id: $scope.copysettlementArr.project_id,
    ...
    $scope.NewlyAdded = function(callback) {
        $scope.copysettlementArr = {
            start_date: parents.currentDate.MonthTime(3),
            end_date: parents.currentDate.MonthTime(1),
            cp_id: '',
            project_id: '',
            product_id: '',
            closing_id: '',
    ...
    $scope.modalSearch1 = function() {
        ...
        $http.get(url + 'admin/finance_profit_report/checkReportInTime', {
            headers: {
                token: parents.token()
            },
    ...
    function initTable() {
        ...
        window.billFun = {
            ...
            //    $scope.collectUrl =
            //        'admin/finance_profit_report/updateIssue/' + row.id;
            //    $scope.collectData = {};
            ...
            'click .closing': function(event, value, row, index) {
                $http.defaults.headers.common = { token: parents.token() };
                $http
                    .get(url + 'admin/finance_profit_report/getIssueNameAndId')
                    .then(function(data) {
                        $scope.modalUnit = data.data.data;
                        $scope.collectTitle = '发行主体';
                        $scope.collectUrl = 'admin/finance_profit_report/updateIssue/' + row.id;
                        $scope.collectData = {};

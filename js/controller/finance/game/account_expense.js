...
app.controller('accountExpenseCtrl', function($http, $scope, $q, $timeout) {
    ...
    if (!state) {
        $scope.index = parents.authList('accountexpense-index'); //查看列表权限
        $scope.read = parents.authList('accountexpense-read'); //查看详情权限
        $scope.update = parents.authList('accountexpense-update'); //编辑操作
        $scope.save = parents.authList('accountexpense-save'); //添加操作
        $scope.delete = parents.authList('accountexpense-delete'); //删除操作
        $scope.deletes = parents.authList('accountexpense-deletes'); //批量删除操作
        $scope.enables = parents.authList('accountexpense-enables'); //批量启用禁用操作
        $scope.isexport = parents.authList('accountexpense-export');//导出操作
        $scope.imports = parents.authList('accountexpense-importdata');//导入操作
    }
    $('#query').show();

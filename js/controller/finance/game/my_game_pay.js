...
app.controller('gamePayCtrl', function($http, $scope, $q, $timeout) {
    ...
    if (!state) {
        $scope.index = parents.authList('mygamepay-index'); //查看列表权限
        $scope.read = parents.authList('mygamepay-read'); //查看详情权限
        $scope.update = parents.authList('mygamepay-update'); //编辑操作
        $scope.save = parents.authList('mygamepay-save'); //添加操作
        $scope.delete = parents.authList('mygamepay-delete'); //删除操作
        $scope.deletes = parents.authList('mygamepay-deletes'); //批量删除操作
        $scope.enables = parents.authList('mygamepay-enables'); //批量启用禁用操作
        $scope.isexport = parents.authList('mygamepay-export');//导出操作
        $scope.imports = parents.authList('mygamepay-importdata');//导入操作
    }
    $('#query').show();

...
app.controller('gameQualificationCtrl', function($http, $scope, $q, $timeout) {
    ...
    if (!state) {
        $scope.index = parents.authList('qualification-index'); //查看列表权限
        $scope.read = parents.authList('qualification-read'); //查看详情权限
        $scope.update = parents.authList('qualification-update'); //编辑操作
        $scope.save = parents.authList('qualification-save'); //添加操作
        $scope.delete = parents.authList('qualification-delete'); //删除操作
        $scope.deletes = parents.authList('qualification-deletes'); //批量删除操作
        $scope.enables = parents.authList('qualification-enables'); //批量启用禁用操作
        $scope.isexport = parents.authList('qualification-export');//导出操作
        $scope.imports = parents.authList('qualification-importdata');//导入操作
        $scope.updateinvoiceinfo = parents.authList('qualification-updateinvoiceinfo');//导入操作
    }
    $('#query').show();

...
app.controller('gameServerCtrl', function($http, $scope, $q, $timeout) {
    ...
    if (!state) {
        $scope.index = parents.authList('myserver-index'); //查看列表权限
        $scope.read = parents.authList('myserver-read'); //查看详情权限
        $scope.update = parents.authList('myserver-update'); //编辑操作
        $scope.save = parents.authList('myserver-save'); //添加操作
        $scope.delete = parents.authList('myserver-delete'); //删除操作
        $scope.deletes = parents.authList('myserver-deletes'); //批量删除操作
        $scope.enables = parents.authList('myserver-enables'); //批量启用禁用操作
        $scope.isexport = parents.authList('myserver-export');//导出操作
        $scope.imports = parents.authList('myserver-importdata');//导入操作
        $scope.updateinvoiceinfo = parents.authList('myserver-updateinvoiceinfo');//导入操作
    }
    $('#query').show();
            ...
            onLoadSuccess: function(data) {
                ...
                if (data.total != 0) {
                    ...
                    var tap = function(t_idx) {
                        ...
                        parents.forSome(
                            $scope.copyserverArr,
                            ['amount', 'total_money', 'is_invoice', 'invoice_number'],
                            Number
                        );

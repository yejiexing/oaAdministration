 ...
 */
var app = angular.module('myActivityExpense', []);
app.controller('myActivityExpenseCtrl', function($http, $scope, $q, $timeout) {
    var self = this;
    ...
    if (!state) {
        $scope.index = parents.authList('myactivityexpense-index'); //查看列表权限
        $scope.read = parents.authList('myactivityexpense-read'); //查看详情权限
        $scope.update = parents.authList('myactivityexpense-update'); //编辑操作
        $scope.save = parents.authList('myactivityexpense-save'); //添加操作
        $scope.delete = parents.authList('myactivityexpense-delete'); //删除操作
        $scope.deletes = parents.authList('myactivityexpense-deletes'); //批量删除操作
        $scope.enables = parents.authList('myactivityexpense-enables'); //批量启用禁用操作
        $scope.isexport = parents.authList('myactivityexpense-export');//导出操作
        $scope.imports = parents.authList('myactivityexpense-import');//导入操作
        //$scope.updateinvoiceinfo = parents.authList('myactivityexpense-updateinvoiceinfo');//导入操作
    }
    $('#query').show();
